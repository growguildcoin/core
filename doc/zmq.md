...
The MentaCoin Core daemon can be configured to act as a trusted "border
router", implementing the mentacoin wire protocol and relay, making
...
The ZeroMQ feature in MentaCoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building mentacoind:
...
For instance:
    ...
    $ mentacoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/mentacoind.tx.raw \
...
These options can also be provided in mentacoin.conf.
...
From the perspective of mentacoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into mentacoind directly. Furthermore, no information is
...
using. MentaCoind appends an up-counting sequence number to each
